...
type MotionPropsWithOutStyle = Omit<MotionProps, "style">;

interface AnimatedWaveProps extends MotionPropsWithOutStyle {
  children?: ReactNode;
  phase?: number;
  amplitude?: number;
  speed?: number;
...
const AnimatedWave: React.FC<AnimatedWaveProps> = ({
  ...
  const wavePath = (x: number): string => {
    const y =
      ...
      amplitude * Math.sin(2 * Math.PI * frequency * x + (phase + value));
    return `${x} ${y}`;
  };
    ...
    <AnimatePresence>
      {showWave && (
        <motion.div
          ...
          ref={myDivRef}
          className={`${className === "" ? "animated-wave" : className} `}
        ...
        >
          <motion.svg width={dimensions.width} height={dimensions.height}>
            {children}
            <path
              d={`M0 ${dimensions.height} ${points} L${dimensions.width} ${dimensions.height}`}
              fill={fill || "#000000"}
            />
          </motion.svg>
        </motion.div>
